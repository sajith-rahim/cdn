...
(function () {
    ...
    function init() {
        debugger;
      //const MODEL_PATH = 'masterchief1k.glb';
      const MODEL_PATH = './assets/models/master_cheif1k_arm_stretch.glb';
      ...
      let stacy_txt = new THREE.TextureLoader().load('./assets/models/txt_poly.jpg');
      ...
      function (gltf) {
        ...
        //load other anims
        let glbFileNames = ['idle_checking_hand.glb', 'victory_idle.glb', 'breakdance.glb'];
        for (const file of glbFileNames) {
          //loadAnimationsFromGLBFiles(file);
        } 
  
      
        setTimeout(()=> {
          debugger;
          let idleAnim = possibleAnims.filter(e => e._clip.name !== 'idle_checking_hand');
          if( idleAnim.length > 0){
            idleAnim = idleAnim[0]._clip;
      ...
      function loadAnimationsFromGLBFiles(file){
        try {
          new THREE.GLTFLoader().load( 
            ...
            let _clip = _gltf.animations[0];
            _clip.name = name;
      
            _clip.tracks.splice(3, 3);
            _clip.tracks.splice(9, 3);
      
            _clip = mixer.clipAction(_clip);
            possibleAnims.push(_clip);
            }
          );
        } catch (error) {
    ...
    function playOnClick() {
      ...
      //let animation = possibleAnims[anim];
      ...
      //in case we have multiple anims
    ...
    document.addEventListener('mousemove', function (e) {
      ...
      if (neck && waist) {
        ...
        moveJoint(mousecoords, neck, 30, 30);
        moveJoint(mousecoords, head, 30, 30);
        moveJoint(mousecoords, waist, 10, 10);
  
        //moveJoint(mousecoords, leftForeArm, -30, 30 );
        //(mousecoords, rightForeArm, 30, 30);
        //moveJoint(mousecoords, leftArm, 0, 60);
        //moveJoint(mousecoords, rightArm, 0, 60);
  
  
        moveJoint(mousecoords, leftLeg, -1, -1);
        moveJoint(mousecoords, rightLeg, 0, 30);
  
        if(!currentlyAnimating && idle){
          //console.log("idle triggered by mousemove")
          idle.play();
        }
    ...
    function moveJoint(mouse, joint, xDegreeLimit ,yDegreeLimit) {
      let degrees = getMouseDegrees(mouse.x, mouse.y, xDegreeLimit, yDegreeLimit);
      joint.rotation.y = THREE.Math.degToRad(degrees.x);
      
      if([leftArm.name, rightArm.name].includes(joint.name)){
    ...
    function getMouseDegrees(x, y, xDegreeLimit, yDegreeLimit) {
      let dx = 0,
      dy = 0,
      xdiff,
      xPercentage,
      ...
      if (x <= w.x / 2) {
        ...
        dx = xDegreeLimit * xPercentage / 100 * -1;
      }
      ...
      if (x >= w.x / 2) {
        xdiff = x - w.x / 2;
        xPercentage = xdiff / (w.x / 2) * 100;
        dx = xDegreeLimit * xPercentage / 100;
      }
      ...
      if (y <= w.y / 2) {
        ...
        dy = yDegreeLimit * 0.5 * yPercentage / 100 * -1;
      }
      // Down (Rotates neck down between 0 and degreeLimit)
      if (y >= w.y / 2) {
        ydiff = y - w.y / 2;
        yPercentage = ydiff / (w.y / 2) * 100;
        dy = yDegreeLimit * yPercentage / 100;
      }
      return { x: dx, y: dy };
    }
